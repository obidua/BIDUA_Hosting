...
async def seed_countries():
    """Seed countries data into the database"""
    ...
    async with engine.begin() as conn:
        ...
        try:
            ...
            existing_countries = await country_service.get_countries_count(async_session, active_only=False)
            
            if existing_countries > 0:
                print(f"⚠️  Countries table already has {existing_countries} records. Skipping seed...")
                return
            
            # Create CountryCreate objects
            countries_to_create = [CountryCreate(**country_data) for country_data in COUNTRIES_DATA]
            
            # Bulk create countries
            print(f"📝 Creating {len(countries_to_create)} countries...")
            created_countries = await country_service.bulk_create_countries(async_session, countries_to_create)
            
            print(f"✅ Successfully created {len(created_countries)} countries!")
            
            # Print some statistics
            active_count = await country_service.get_countries_count(async_session, active_only=True)
            total_count = await country_service.get_countries_count(async_session, active_only=False)
